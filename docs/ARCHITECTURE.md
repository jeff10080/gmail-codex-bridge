...
Sans route, le destinataire determine le projet. L'adresse Gmail nue selectionne `default_project`; un suffixe `+alias` selectionne la cle correspondante dans `[projects]`. Si le projet existe, le SDK appelle `startThread` dans son repertoire et la nouvelle route est enregistree avant la reponse. Si le projet est inconnu, le message est place en quarantaine et Codex n'est pas lance.

Python lance `scripts/codex-runner.mjs`, qui utilise le SDK officiel `@openai/codex-sdk`, appelle `startThread(options)` ou `resumeThread(threadId, options)`, puis execute `thread.run(prompt)`. Le resultat final et l'identifiant de la tache sont renvoyes au service en JSONL.
